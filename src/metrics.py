...
def calculate_seasonal_error(
    past_data: list[torch.tensor], seasonality: int
) -> torch.tensor:
    seasonal_errors_per_series = []
    for series in past_data:
        error = _calculate_seasonal_error(series, seasonality)
...
def abs_error(y_hat: torch.tensor, y: torch.tensor, dim=1) -> torch.tensor:
    return (
        (y_hat - y).abs().sum(dim=dim)
        if dim is not None
        else (y_hat - y).abs().sum()
    )
...
def nd(y_hat: torch.tensor, y: torch.tensor, dim: int = 1) -> torch.tensor:
    return (
        abs_error(y_hat, y, dim=dim).sum() / abs_target_sum(y, dim=dim).sum()
    )
